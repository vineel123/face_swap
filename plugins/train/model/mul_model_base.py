...
class ModelBase():
    ...
    #verrified
    #adds network , file_name to store , as dict with key"_i"
    ...
    #verified
    #creates autoencoder for each side and stores it in a dict
    ...
    #verified
    #stores models input_shape in state
    ...
    #verified
    #stores model output shape in state 
    ...
    #verified
    #lmask_loss_function which we will not be using
    ...
    #changed
    #return autoencoder of given side 
    def converter(self, side1):
        """ Converter for autoencoder models """
        logger.debug("Getting Converter: (swap: %s)", swap)
        model = self.predictors[side1]
        if self.predict:
    ...
    #verified 
    #retunes current training iteration
    @property
    def iterations(self):
    ...
    #verfied
    #checks and loads the model in the given directory
