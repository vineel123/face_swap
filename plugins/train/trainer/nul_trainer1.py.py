...
class TrainerBase():
    ...
    #verified
    #calls train_one_batch , compile_sample(for preview)    
...
class Samples():
    ...
    def show_sample(self):
        ...
        for side, samples in self.images.items():
            predictions = [preds["{}_{}".format(side, str(other_side))] for other_side in range(len(self.images.items())) ]
            display = self.to_full_frame(side, samples, predictions)
            headers[side] = self.get_headers(side,len(self.images.items()) ,display[0].shape[1])
            figures[side] = np.stack([i for i in display ], axis=1)
        ...
        header = np.concatenate([headers[str(i)] for i in range(len(headers)) ] ,axis=1)
        figure = np.concatenate([figures[str(i)] for i in range(len(figures))], axis=0)
        height = int(figure.shape[0] / width)
        figure = figure.reshape((width, height) + figure.shape[1:])
        figure = stack_images(figure)
        print(f"----------------------------------------{len(headers)}-------------------------")
        figure = np.vstack((figure))
    ...
    #changed
    def get_headers(self, side, size , width):
        """ Set headers for images """
        side = side.upper()
        height = int(64 * self.scaling)
        total_width = width * size
        logger.debug("height: %s, total_width: %s", height, total_width)
        font = cv2.FONT_HERSHEY_SIMPLEX  # pylint: disable=no-member
        texts = ["Target {}".format(side)]+["{} > {}".format(side, str(other_side)) for other_side in range(size)]
